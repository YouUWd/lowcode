<template>
  ...
  <g>
    ...
    >
      ...
      >
        {{ data?.rel?.name || `${data?.rel?.sourceTable} → ${data?.rel?.targetTable}` }}
      </div>
    </foreignObject>
  </g>
</template>
...
const isActive = computed(() =>
  hovered.value || activeEdgeId.value === props.id
)
...
// 基数标签：根据 relationType 显示 N 或 1
const nLabel = computed(() => {
  const rt = props.data?.rel?.relationType ?? ''
  if (rt === 'ONE_TO_ONE') return '1'
  if (rt === 'MANY_TO_ONE') return '1'
  return 'N' // ONE_TO_MANY / 默认
})
