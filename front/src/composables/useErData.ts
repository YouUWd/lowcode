...
import { fetchMetaSchema, saveMetaSchema, ApiError } from '../api'
...
export function useErData() {
  const schema = ref<ErSchema>({ datasources: [], relations: [] })
  const loading = ref(false)
  const saving  = ref(false)
  const error   = ref<string | null>(null)

  // 所有物理表（跨 datasource 展开）
  const allTables = computed<ErTable[]>(() => {
    const list: ErTable[] = []
    ;(schema.value?.datasources || []).forEach(ds => {
      if (ds.tables) list.push(...ds.tables)
    })
    return list
  })
  ...
  const loadSchema = async (): Promise<{ nodes: Node[]; edges: Edge[] }> => {
    ...
    try {
      ...
      const data: ErSchema = res.data || { datasources: [], relations: [] }
  ...
  const addRelation = async (rel: Omit<ErRelation, 'id'>): Promise<boolean> => {
    ...
    // 找字段 ID
    const srcTable = tablesMap.value[rel.sourceTable]
    const tgtTable = tablesMap.value[rel.targetTable]
    const srcField = srcTable?.fields.find(f => f.columnName === rel.sourceColumn)
    const tgtField = tgtTable?.fields.find(f => f.columnName === rel.targetColumn)

    const newRel: ErRelation = {
      id: -Date.now(), // 临时 ID，后端保存后会替换
      ...rel,
      sourceFieldId: srcField?.id ?? 0,
      targetFieldId: tgtField?.id ?? 0,
    }
  ...
  const updateTableDisplayName = async (tableName: string, displayName: string): Promise<boolean> => {
    for (const ds of schema.value.datasources) {
      const t = ds.tables?.find(t => t.tableName === tableName)
      if (t) { t.displayName = displayName; break }
    }
    return saveSchema()
  }
  ...
  const updatePrimaryColumn = async (tableName: string, primaryColumn: string): Promise<boolean> => {
    for (const ds of schema.value.datasources) {
      const t = ds.tables?.find(t => t.tableName === tableName)
      if (t) { t.primaryColumn = primaryColumn; break }
    }
    return saveSchema()
  }
